  ...
  <div class="app-container">
    <el-card shadow="never" body-class="search-card">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
        ...
        <el-form-item label="关联本平台用户ID" prop="userId">
          <el-input
            v-model="queryParams.userId"
            placeholder="请输入关联本平台用户ID"
            clearable
            @keyup.enter="handleQuery"
          />
        </el-form-item>
    ...
    <el-card shadow="never" class="mt10">
      ...
      <el-table-column label="关联本平台用户ID" align="center" prop="userId" />
      ...
      <el-table-column label="同步状态：待同步/已同步/失败" align="center" prop="syncStatus" />
      ...
      <el-table-column label="原始数据" align="center" prop="jsonData" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="xiaoeordersRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="小鹅通订单号" prop="xiaoeOrderNo">
          <el-input v-model="form.xiaoeOrderNo" placeholder="请输入小鹅通订单号" />
        </el-form-item>
        <el-form-item label="关联本平台用户ID" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入关联本平台用户ID" />
        </el-form-item>
        <el-form-item label="小鹅通订单状态" prop="orderState">
          <el-input v-model="form.orderState" placeholder="请输入小鹅通订单状态" />
        </el-form-item>
        <el-form-item label="实际支付金额" prop="actualFee">
          <el-input v-model="form.actualFee" placeholder="请输入实际支付金额" />
        </el-form-item>
        <el-form-item label="商品名称" prop="goodsName">
          <el-input v-model="form.goodsName" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="商品类型" prop="goodsType">
          <el-select v-model="form.goodsType" multiple filterable remote reserve-keyword remote-show-suffix
            placeholder="请选择商品类型"
            :remote-method="remoteMethodGoodsType"
            :loading="loadingGoodsType"
          >
            <el-option v-for="item in optionsGoodsType" :key="item.value"
              :label="item.label" :value="item.value"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="SPU类型" prop="spuType">
          <el-select v-model="form.spuType" multiple filterable remote reserve-keyword remote-show-suffix
            placeholder="请选择SPU类型"
            :remote-method="remoteMethodSpuType"
            :loading="loadingSpuType"
          >
            <el-option v-for="item in optionsSpuType" :key="item.value"
              :label="item.label" :value="item.value"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="支付状态" prop="payState">
          <el-input v-model="form.payState" placeholder="请输入支付状态" />
        </el-form-item>
        <el-form-item label="支付方式" prop="payType">
          <el-select v-model="form.payType" multiple filterable remote reserve-keyword remote-show-suffix
            placeholder="请选择支付方式"
            :remote-method="remoteMethodPayType"
            :loading="loadingPayType"
          >
            <el-option v-for="item in optionsPayType" :key="item.value"
              :label="item.label" :value="item.value"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="第三方交易号" prop="tradeNo">
          <el-input v-model="form.tradeNo" placeholder="请输入第三方交易号" />
        </el-form-item>
        <el-form-item label="小鹅通侧创建时间" prop="xiaoeCreateTime">
          <el-date-picker clearable
            v-model="form.xiaoeCreateTime"
            type="date"
            value-format="YYYY-MM-DD"
            placeholder="请选择小鹅通侧创建时间">
          </el-date-picker>
        ...
        <el-form-item label="本地订单号" prop="orderNo">
          <el-input v-model="form.orderNo" placeholder="请输入本地订单号" />
        </el-form-item>
